...
export default function StudentAppointments({ appointments, slots }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        post(route('student.appointments.store'), { onSuccess: () => { setShowBook(false); reset(); } });
    };

    const cancel = (id) => {
        if (confirm('Cancel this appointment?')) router.post(route('student.appointments.cancel', id));
    };
    ...
    return (
        <StudentLayout title="My Appointments">
            ...
            <div className="page-header flex-col sm:flex-row items-start sm:items-center gap-3">
                ...
                <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                    <Link href={route('student.appointments.index')} className="btn-secondary justify-center w-full sm:w-auto">
                            ...
                            {appointments.data.map(a => (
                                <tr key={a.id}>
                                    ...
                                    <td>
                                        {statusBadge(a.status)}
                                        {a.status === 'declined' && a.decline_reason && (
                                            <p className="text-xs text-red-500 mt-1">Reason: {a.decline_reason}</p>
                                        )}
                                    </td>
                                    <td className="text-gray-500 text-xs max-w-xs truncate">{a.notes || '—'}</td>
                                    <td>
                                        {['pending', 'approved'].includes(a.status) && (
                            ...
                            {!appointments.data.length && (
                                <tr>
                                    <td colSpan={6} className="text-center text-gray-400 py-8">
                                        No appointments yet.
                                        <button onClick={() => setShowBook(true)} className="ml-2 text-clinic-600 hover:underline">Book now</button>
                                    </td>
                                </tr>
                            )}
                        </tbody>
                ...
                <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center backdrop-blur-sm bg-black/30 p-0 sm:p-4">
                    <div className="bg-white rounded-t-2xl sm:rounded-xl shadow-xl p-6 w-full sm:max-w-md max-h-[90vh] overflow-y-auto">
                        ...
                        <form onSubmit={submit} className="space-y-4">
                            ...
                            <div>
                                <label htmlFor="purpose" className="label">Purpose / Reason</label>
                            ...
                            <div>
                                <label htmlFor="notes" className="label">Additional Notes <span className="text-gray-400 font-normal">(optional)</span></label>
                                <textarea id="notes" name="notes" value={data.notes} onChange={e => setData('notes', e.target.value)}
                                    className="input" rows={2} placeholder="Any other details…" />
                            </div>
