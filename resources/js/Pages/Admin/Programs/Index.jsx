                        ...
                        <div key={p.id}>
                            ...
                            <div className="px-6 py-4 flex items-center justify-between">
                                <div className="flex items-center gap-4 flex-1">
                                    <div className="w-10 h-10 bg-clinic-100 text-clinic-700 rounded-lg flex items-center justify-center font-bold text-sm">
                                        {p.code}
                                    </div>
                                ...
                                <div className="flex items-center gap-3 ml-4">
                                    <button onClick={() => setExpanded(expanded === p.id ? null : p.id)}
                                        className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-clinic-600 transition-colors px-2 py-1 rounded hover:bg-clinic-50">
                                        ...
                                        <>
                                            ...
                                            <div className="relative mb-3">
                                                ...
                                                <input
                                                    ...
                                                    className="input pl-9 w-full max-w-sm text-sm"
                                                ...
                                                <table className="min-w-full divide-y divide-gray-200 text-sm">
                                                    <thead className="bg-white">
                                                        <tr>
                                                            ...
                                                            <th className="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase">Student ID</th>
                                                            <th className="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase">Year</th>
                                                            <th className="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase">Block</th>
                                                            <th className="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase">Status</th>
                                                            <th className="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase">Pregnant</th>
                                                    ...
                                                    <tbody className="bg-white divide-y divide-gray-100">
                                                        {filtered.length ? filtered.map(sp => (
                                                            <tr key={sp.id} className="hover:bg-gray-50">
                                                                <td className="px-4 py-2 font-medium text-gray-900">{sp.user?.name}</td>
                                                                <td className="px-4 py-2 text-gray-500">{sp.user?.email}</td>
                                                                <td className="px-4 py-2 text-gray-500">{sp.student_id || '—'}</td>
                                                                <td className="px-4 py-2 text-gray-500">{sp.year_level ? `Year ${sp.year_level}` : '—'}</td>
                                                                <td className="px-4 py-2 text-gray-500">{sp.block || '—'}</td>
                                                            ...
                                                            <tr><td colSpan={7} className="px-4 py-6 text-center text-gray-400 text-sm">No students match "{query}".</td></tr>
                                                        )}
                                                    </tbody>
                                                </table>
                                            </div>
