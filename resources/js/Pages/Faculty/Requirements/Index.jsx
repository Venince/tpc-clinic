                            ...
                            <div className="card-body flex items-start justify-between gap-4">
                                <div className="flex items-start gap-4 flex-1">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        {req && <div className="mt-2"><p className="text-xs text-gray-500">File: <span className="font-medium">{req.original_filename}</span></p>{req.rejection_reason && <p className="text-xs text-red-500 mt-1">Reason: {req.rejection_reason}</p>}</div>}
                                    </div>
                                </div>
                                <div className="flex items-center gap-3 flex-shrink-0">
                                    {statusBadge(req?.approval_status)}
                                    <button onClick={()=>openUpload(type)} className="btn-secondary btn-sm"><ArrowUpTrayIcon className="w-4 h-4 mr-1"/>{req?'Re-upload':'Upload'}</button>
                                </div>
                            </div>
                        </div>
                    );
            ...
            {uploading && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                    <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-md">
                        <h3 className="font-semibold mb-4">Upload: {uploading.name}</h3>
