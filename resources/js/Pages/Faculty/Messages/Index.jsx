...
export default function FacultyMessagesIndex({ conversations, contacts }) {
    ...
    const handleDelete = () => {
        router.delete(route('faculty.messages.destroy', confirmDelete.id), {
                ...
                <div className="divide-y divide-gray-100">
                    {conversations.data?.map(c => (
                        <div key={c.id} className="flex items-center gap-2 px-6 py-4 hover:bg-gray-50 transition-colors group">
                            <Link href={route('faculty.messages.show', c.id)} className="flex items-center justify-between flex-1 min-w-0">
