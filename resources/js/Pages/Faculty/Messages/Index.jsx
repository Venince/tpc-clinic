...
export default function FacultyMessagesIndex({ conversations, contacts }) {
    ...
    const submit = (e) => { e.preventDefault(); post(route('faculty.messages.store'), { onSuccess: () => { setShowNew(false); reset(); } }); };
            ...
            <div className="card">
                <div className="divide-y divide-gray-100">
                    {conversations.data?.map(c => (
                        <Link key={c.id} href={route('faculty.messages.show', c.id)} className="block px-6 py-4 hover:bg-gray-50 transition-colors">
